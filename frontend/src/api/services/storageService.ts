...
export const fileService = {
  ...
  viewFile: (file_path: string) => {
    ...
    // Use native fetch to avoid Axios URL encoding
    const url = `http://127.0.0.1:8000/api/files/view/${cleanPath}`;
  ...
  // Get shared sent items (files/folders the user has shared)

  getSharedSentItems: (type?: "internal" | "external") => {
    const params: any = {};
    ...
    return client.get("/files/shared/sent/", { params });
  },

  // Get shared received items (files/folders shared with the user)

  getSharedReceivedItems: () => {
    return client.get("/files/shared/received/");
  },

  // Get all shared items created by the logged in user

  getSharedItemsList: (type?: "file" | "folder", status?: "active" | "all") => {
    const params: any = {};
    ...
    return client.get("/files/shared-items/", { params });
  },
};
...
export const trashService = {
  // Get trash contents
  getTrash: () => client.get("/files/trash/"),

  // Move items to trash

  moveToTrash: (paths: string[]) =>
    client.post("/files/trash/", {
      paths: paths,
    }),

  // Restore items from trash
  restoreFromTrash: (itemIds: string[], itemType: "file" | "folder") =>
    client.post("/files/trash/restore/", {
      ids: itemIds,
      item_type: itemType,
    }),
  ...
  deletePermanently: (itemIds: string[], itemType: "file" | "folder") =>
    client.delete("/files/trash/delete/", {
      data: {
        ids: itemIds,
        item_type: itemType,
      },
  ...
  emptyTrash: () => client.delete("/files/trash/"),
};
...
export const favoritesService = {
  // Get all favorites
  getFavorites: () => client.get("/files/favorites/"),

  // Add items to favorites
  addToFavorites: (paths: string[]) =>
    client.post("/files/favorites/", { paths }),

  // Remove items from favorites
  removeFromFavorites: (ids: number[]) =>
    client.delete("/files/favorites/", { data: { ids } }),
};
